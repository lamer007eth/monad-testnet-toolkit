...
ERC20_ABI = [
    ...
    {"name":"allowance","stateMutability":"view","type":"function","inputs":[{"name":"owner","type":"address"},{"name":"spender","type":"address"}],"outputs":[{"type":"uint256"}]},
    {"name":"approve","stateMutability":"nonpayable","type":"function","inputs":[{"name":"spender","type":"address"},{"name":"amount","type":"uint256"}],"outputs":[{"type":"bool"}]},
]
ROUTER_ABI = [
    ...
    {"name":"swapExactTokensForETH","stateMutability":"nonpayable","type":"function",
     "inputs":[
...
WMON_ABI = [{"name":"withdraw","stateMutability":"nonpayable","type":"function","inputs":[{"name":"wad","type":"uint256"}],"outputs":[]}]
...
def run():
    ...
    print("✅ Swap Token→MON: Web3 connected")
    ...
    percent_min, percent_max = 100.0, 100.0
    raw = input("📊 Диапазон % (min max), Enter=100 100: ").strip()
    if raw:
        a, b = raw.replace(",", " ").split()
        percent_min, percent_max = float(a), float(b)

    rounds = int(input("🔁 rounds: ").strip())

    def ensure_approve(owner: str, pk: str, need: int):
        allowance = token.functions.allowance(owner, Web3.to_checksum_address(V2_ROUTER)).call()
        if allowance >= need:
            return None
        tx = token.functions.approve(Web3.to_checksum_address(V2_ROUTER), MAX_UINT256).build_transaction({
            "from": owner,
    ...
    for r in range(1, rounds + 1):
        ...
        for addr, pk in items:
            try:
                ...
                if bal <= 0:
                    print(f"⚠️ {_short(wallet)} no {sym}")
                    continue

                p = random.uniform(percent_min, percent_max) / 100.0
                amount_in = int(bal * p)

                ensure_approve(wallet, pk, amount_in)

                path = [token_addr, WMON_from_router]
                quoted_out = router.functions.getAmountsOut(amount_in, path).call()[-1]
                amount_out_min = int(quoted_out * (1 - SLIPPAGE_BPS / 10_000))
                ...
                signed = w3.eth.account.sign_transaction(tx, pk)
                tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction).hex()
                print(f"✅ {_short(wallet)} sell {sym} → MON tx: {tx_hash}")
